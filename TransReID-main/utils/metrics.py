...
class R1_mAP_eval():
    ...
    def match_id(self, distmat, q_pids, g_pids, final_output, max_M=16):
        num_q, num_g = distmat.shape
        if num_g < max_M:
            max_M = num_g
            print("Note: number of gallery samples is quite small, got {}".format(num_g))
        ...
        with open(final_output_path, 'w') as file:
            for q_idx in range(num_q):
                ...
                # not match: 最匹配的分数 > 阈值 or 匹配序列均值 > 2 * 阈值
                if distmat[q_idx][indices[q_idx][0]] > shreshold1 :# or distmat[q_idx][inds_needed].mean() > (shreshold1 * 1.6) :
                    cnt_not_found += 1
