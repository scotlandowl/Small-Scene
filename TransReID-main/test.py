...
if __name__ == "__main__":
    ...
    if cfg.DATASETS.NAMES == 'VehicleID':
        ...
        #                 'camid': data[2][i],
        #                 'camids': data[3][i],
        #                 'target_view': data[4][i],
        #                 'imgpath': data[5][i]
        #             }
        #             data_array.append(item)
        #     query_loaders.append(data_array)
        #     num_querys.append(query_loader)
            
        #     if (i != 0 and i % 11 == 0) or flag == 1:
        #         print('do')
        #         for j in range(len(num_querys)):
        #             query_loader = query_loaders[j]
        #             num_query = num_querys[j]
        #             evaluator.change(num_query=num_query)
        #             do_inference(evaluator, model, query_loader, num_query, query_path_name=q_path)